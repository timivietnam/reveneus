...
import { Day_Type } from 'utils/dataCalendarUtil';
...
function Day({ day, locale, disabledBeforeToday, style }: Props) {
  ...
  const todayColor = style?.todayColor || '#6941C6';
    ...
    <>
      ...
      {date ? (
        <View style={markStyle}>
          <Text style={[{ fontSize: 15,fontWeight:'600' }, dayStyle, style?.dayText]}>
            {moment(date).date()}
          </Text>
        </View>
      ) : null}
      {isToday ? (
        <Text style={[{ fontSize: 12,fontWeight:'bold' }, { color: todayColor }]}>
          {locale.today}
        </Text>
      ) : null}
    </>
